...
class StammdatenImportServiceTest {
    ...
    void beforeEach() {
        ...
        Mockito.reset(this.multipartFile, this.csvRepository, this.staedtebaulicheOrientierungswertRepository, this.sobonOrientierungswertSozialeInfrastrukturRepository);
    }

    @Test
    void importStaedtebaulicheOrientierungswerte() throws IOException, CsvRequiredFieldEmptyException, CsvDataTypeMismatchException, CsvAttributeErrorException, FileImportFailedException {
        ...
        Mockito.when(this.csvRepository.readAllStaedtebaulicheOrientierungswertCsv(Mockito.any(InputStreamReader.class))).thenReturn(List.of(csvEntry));
        ...
        Mockito.verify(this.csvRepository, Mockito.times(1)).readAllStaedtebaulicheOrientierungswertCsv(Mockito.any(InputStreamReader.class));
        Mockito.verify(this.staedtebaulicheOrientierungswertRepository, Mockito.times(1)).saveAll(entities);
    }

    @Test
    void importStaedtebaulicheOrientierungswerteException() throws IOException, CsvRequiredFieldEmptyException, CsvDataTypeMismatchException {
        InputStream inputStream = new ByteArrayInputStream("the-file".getBytes());
        Mockito.when(this.multipartFile.getInputStream()).thenReturn(inputStream);
        Mockito.doThrow(new CsvDataTypeMismatchException()).when(this.csvRepository).readAllStaedtebaulicheOrientierungswertCsv(Mockito.any(InputStreamReader.class));
        Assertions.assertThrows(CsvAttributeErrorException.class, () -> this.stammdatenImportService.importStaedtebaulicheOrientierungswerte(this.multipartFile));
        Mockito.reset(this.csvRepository, this.multipartFile);

        inputStream = new ByteArrayInputStream("the-file".getBytes());
        Mockito.when(this.multipartFile.getInputStream()).thenReturn(inputStream);
        Mockito.doThrow(new CsvRequiredFieldEmptyException()).when(this.csvRepository).readAllStaedtebaulicheOrientierungswertCsv(Mockito.any(InputStreamReader.class));
        Assertions.assertThrows(CsvAttributeErrorException.class, () -> this.stammdatenImportService.importStaedtebaulicheOrientierungswerte(this.multipartFile));
        Mockito.reset(this.csvRepository, this.multipartFile);

        Mockito.when(this.multipartFile.getInputStream()).thenThrow(new IOException());
        Assertions.assertThrows(FileImportFailedException.class, () -> this.stammdatenImportService.importStaedtebaulicheOrientierungswerte(this.multipartFile));
    }

    @Test
    void importSoBoNOrientierungswerteSozialeInfrastruktur() throws IOException, CsvRequiredFieldEmptyException, CsvDataTypeMismatchException, CsvAttributeErrorException, FileImportFailedException {
        final InputStream inputStream = new ByteArrayInputStream("the-file".getBytes());
        Mockito.when(this.multipartFile.getInputStream()).thenReturn(inputStream);
        ...
        Mockito.when(this.csvRepository.readAllSobonOrientierungswertSozialeInfrastrukturCsv(Mockito.any(InputStreamReader.class))).thenReturn(List.of(csvEntry));
        ...
        Mockito.verify(this.csvRepository, Mockito.times(1)).readAllSobonOrientierungswertSozialeInfrastrukturCsv(Mockito.any(InputStreamReader.class));
        Mockito.verify(this.sobonOrientierungswertSozialeInfrastrukturRepository, Mockito.times(1)).saveAll(entities);
    }

    @Test
    void importSoBoNOrientierungswerteSozialeInfrastrukturException() throws IOException, CsvRequiredFieldEmptyException, CsvDataTypeMismatchException {
        InputStream inputStream = new ByteArrayInputStream("the-file".getBytes());
        Mockito.when(this.multipartFile.getInputStream()).thenReturn(inputStream);
        Mockito.doThrow(new CsvDataTypeMismatchException()).when(this.csvRepository).readAllSobonOrientierungswertSozialeInfrastrukturCsv(Mockito.any(InputStreamReader.class));
        Assertions.assertThrows(CsvAttributeErrorException.class, () -> this.stammdatenImportService.importSobonOrientierungswerteSozialeInfrastruktur(this.multipartFile));
        Mockito.reset(this.csvRepository, this.multipartFile);

        inputStream = new ByteArrayInputStream("the-file".getBytes());
        Mockito.when(this.multipartFile.getInputStream()).thenReturn(inputStream);
        Mockito.doThrow(new CsvRequiredFieldEmptyException()).when(this.csvRepository).readAllSobonOrientierungswertSozialeInfrastrukturCsv(Mockito.any(InputStreamReader.class));
        Assertions.assertThrows(CsvAttributeErrorException.class, () -> this.stammdatenImportService.importSobonOrientierungswerteSozialeInfrastruktur(this.multipartFile));
        Mockito.reset(this.csvRepository, this.multipartFile);

        Mockito.when(this.multipartFile.getInputStream()).thenThrow(new IOException());
        Assertions.assertThrows(FileImportFailedException.class, () -> this.stammdatenImportService.importSobonOrientierungswerteSozialeInfrastruktur(this.multipartFile));
    }

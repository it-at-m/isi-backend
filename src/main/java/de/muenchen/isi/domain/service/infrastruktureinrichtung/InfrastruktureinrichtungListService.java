...
public class InfrastruktureinrichtungListService {
    ...
    public InfrastruktureinrichtungListElementsModel getInfrastruktureinrichtungListElements() {
        ...
        // Lesen und Mappen
        this.kinderkrippeService.getKinderkrippen()
            .stream()
            .map(
                this.infrastruktureinrichtungListElementDomainMapper::kinderkrippeModel2InfrastruktureinrichtungListElementModel
            )
            .forEach(listElements::add);
        this.kindergartenService.getKindergaerten()
            .stream()
            .map(
                this.infrastruktureinrichtungListElementDomainMapper::kindergartenModel2InfrastruktureinrichtungListElementModel
            )
            .forEach(listElements::add);
        this.hausFuerKinderService.getHaeuserFuerKinder()
            .stream()
            .map(
                this.infrastruktureinrichtungListElementDomainMapper::hausFuerKinderModel2InfrastruktureinrichtungListElementModel
            )
            .forEach(listElements::add);
        this.gsNachmittagBetreuungService.getGsNachmittagBetreuungen()
            .stream()
            .map(
                this.infrastruktureinrichtungListElementDomainMapper::gsNachmittagBetreuungModel2InfrastruktureinrichtungListElementModel
            )
            .forEach(listElements::add);
        this.grundschuleService.getGrundschulen()
            .stream()
            .map(
                this.infrastruktureinrichtungListElementDomainMapper::grundschuleModel2InfrastruktureinrichtungListElementModel
            )
            .forEach(listElements::add);
        this.mittelschuleService.getMittelschulen()
            .stream()
            .map(
                this.infrastruktureinrichtungListElementDomainMapper::mittelschuleModel2InfrastruktureinrichtungListElementModel
            )
            .forEach(listElements::add);

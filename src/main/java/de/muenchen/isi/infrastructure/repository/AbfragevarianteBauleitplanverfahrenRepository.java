...
public interface AbfragevarianteBauleitplanverfahrenRepository
    ...
    @Query(
        value = "SELECT abfragevarianten_bauleitplanverfahren_id FROM abfragevariantebauleitplanverfahren WHERE id = ?1",
        nativeQuery = true
    )
    Optional<String> findAbfrageIdForAbfragevarianteById(final String id);

    @Query(
        value = "SELECT abfragevarianten_sachbearbeitung_bauleitplanverfahren_id FROM abfragevariantebauleitplanverfahren WHERE id = ?1",
        nativeQuery = true
    )

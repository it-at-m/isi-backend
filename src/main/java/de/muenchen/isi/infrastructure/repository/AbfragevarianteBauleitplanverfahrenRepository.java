...
public interface AbfragevarianteBauleitplanverfahrenRepository
    ...
    @Query(
        value = "select abfragevarianten_bauleitplanverfahren_id from abfragevariante_bauleitplanverfahren where CAST(id as uuid) = CAST(:id as uuid)",
        nativeQuery = true
    )
    Optional<UUID> findAbfrageIdForAbfragevarianteById(final UUID id);

    @Query(
        value = "select abfragevarianten_sachbearbeitung_bauleitplanverfahren_id from abfragevariante_bauleitplanverfahren where CAST(id as uuid) = CAST(:id as uuid)",
        nativeQuery = true
    )

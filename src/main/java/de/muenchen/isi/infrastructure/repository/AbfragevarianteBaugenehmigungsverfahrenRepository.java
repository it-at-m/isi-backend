...
public interface AbfragevarianteBaugenehmigungsverfahrenRepository
    ...
    @Query(
        value = "select abfragevarianten_baugenehmigungsverfahren_id from abfragevariante_baugenehmigungsverfahren where id = CAST(?1 AS uuid)",
        nativeQuery = true
    )
    Optional<UUID> findAbfrageIdForAbfragevarianteById(final UUID id);

    @Query(
        value = "select abfragevarianten_sachbearbeitung_baugenehmigungsverfahren_id from abfragevariante_baugenehmigungsverfahren where id = CAST(?1 AS uuid)",
        nativeQuery = true
    )

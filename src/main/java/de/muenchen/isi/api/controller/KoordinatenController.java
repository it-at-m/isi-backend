...
public class KoordinatenController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK -> Transformation der Koordinate von WGS84 zu UTM32 war erfolgreich."),
            @ApiResponse(responseCode = "555", description = "CUSTOM INTERNAL SERVER ERROR -> Bei der Transformation der Koordinate ist ein Fehler aufgetreten.", content = @Content(schema = @Schema(implementation = InformationResponseDto.class)))
    })
    @PreAuthorize("hasAuthority(T(de.muenchen.isi.security.AuthoritiesEnum).ISI_BACKEND_KOORDINATEN_TRANSFORM.name())")
    public ResponseEntity<UtmDto> wgs84toUtm32(@RequestBody @Valid @NotNull final Wgs84Dto wgs84Dto) throws KoordinatenException {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK -> Transformation der Koordinate von UTM32 zu WGS84 war erfolgreich."),
            @ApiResponse(responseCode = "555", description = "CUSTOM INTERNAL SERVER ERROR -> Bei der Transformation der Koordinate ist ein Fehler aufgetreten.", content = @Content(schema = @Schema(implementation = InformationResponseDto.class)))
    })
    @PreAuthorize("hasAuthority(T(de.muenchen.isi.security.AuthoritiesEnum).ISI_BACKEND_KOORDINATEN_TRANSFORM.name())")
    public ResponseEntity<Wgs84Dto> utm32ToWgs84(@RequestBody @Valid @NotNull final UtmDto utmDto) throws KoordinatenException {

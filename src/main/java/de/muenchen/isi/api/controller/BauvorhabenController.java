...
public class BauvorhabenController {
    ...
    @Operation(summary = "Lade alle Bauvorhaben", description = "Das Ergebnis wird nach der Größe des Grundstückes absteigend sortiert")
    @ApiResponses(value = {
    ...
    public ResponseEntity<List<BauvorhabenDto>> getBauvorhaben() {
        final List<BauvorhabenDto> bauvorhabenList =
                this.bauvorhabenService.getBauvorhaben().stream()
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK"),
            @ApiResponse(responseCode = "404", description = "NOT FOUND -> Bauvorhaben mit dieser ID nicht vorhanden.", content = @Content(schema = @Schema(implementation = InformationResponseDto.class)))
    })
    @PreAuthorize("hasAuthority(T(de.muenchen.isi.security.AuthoritiesEnum).ISI_BACKEND_READ_BAUVORHABEN.name())")
    public ResponseEntity<BauvorhabenDto> getBauvorhabenById(@PathVariable @NotNull final UUID id) throws EntityNotFoundException {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "NOT FOUND -> Bauvorhaben mit dieser ID nicht vorhanden.", content = @Content(schema = @Schema(implementation = InformationResponseDto.class))),
            @ApiResponse(responseCode = "409", description = "CONFLICT -> Das Bauvorhaben wird durch Abfragen referenziert.", content = @Content(schema = @Schema(implementation = InformationResponseDto.class)))

    })
    @Transactional
    @PreAuthorize("hasAuthority(T(de.muenchen.isi.security.AuthoritiesEnum).ISI_BACKEND_DELETE_BAUVORHABEN.name())")
    public ResponseEntity<Void> deleteBauvorhaben(@PathVariable @NotNull final UUID id) throws EntityNotFoundException, EntityIsReferencedException {
